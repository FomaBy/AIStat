...
- `GET /api/connection` — статус для кабинета. Токен не возвращается никогда.
  При выключенной функции возвращает `{"status":"disabled"}`.
- `POST /api/connection` — принять/заменить токен. Требуются по порядку:
  ...
  валидные `token`, `server_url` (пиннится к официальному хосту), необязательный
  `workspace_label`; **и свежий heartbeat worker'а** (иначе токен не сохраняется).
...
- **Только официальный хост.** Пользовательский `server_url` не публикуется:
  единственный принимаемый и сохраняемый хост — `AISTAT_MULTICA_OFFICIAL_URL`
  (по умолчанию `https://multica.ai`). Пустое значение подставляет официальный
  хост; любой другой хост, поддомен-суффикс, IP-литерал, порт, credentials или
  `http` отвергаются (`422`). Так «отравленный» `server_url` (в т.ч. через
  redirect/DNS-трюк на нём) не может увести будущее использование PAT в сторону.
...
опционально `AISTAT_MULTICA_OFFICIAL_URL` (по умолчанию `https://multica.ai`),
