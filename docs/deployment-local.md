...
- Каждая развёртка запускает только дашборд (API + UI) со своим `AISTAT_PORT` и
  своей SQLite-БД (`data/aistat.db` внутри своего клона) — ветки не делят
  состояние и не запускают Multica poller/publisher. Команду задаёт сам
  `local_deploy.sh serve <dev|main>`, а **не** `./run.sh` из клона: на старой
  ветке (например, пока `main` не обновлён) `run.sh` дополнительно поднимает
  `aistat.poller`, и это был бы второй сборщик рядом с `com.aistat.runtime`.
...
Манифесты launchd переписываются только `install` (`sync`/`release` лишь
перезапускают уже загруженное задание), поэтому после обновления самого
`local_deploy.sh` выполните `install` ещё раз — иначе агенты продолжат
исполнять команду из старого манифеста.
...
Одна команда. Порядок строго безопасный — **сначала проверка, потом
публикация**:

1. вычисляется SHA кандидата (по умолчанию `origin/dev`, либо `--from <ref>`;
   при `--no-promote` — текущий `origin/main`);
2. кандидат **собирается и валидируется в отдельном временном staging-дереве**
   (`git archive` → `scripts/build_cpanel_package.sh` → `compileall`) — ни
   `origin/main`, ни рабочий `main`-клон при этом не трогаются;
3. **только при успехе** SHA пушится в `origin/main` (кроме `--no-promote`),
   затем `main`-клон переводится на этот коммит и `main`-агент перезапускается.

Если сборка или проверка падают — **`origin/main` и работающая `main`-развёртка
остаются нетронутыми** (ничего не публикуется, live-клон не сбрасывается). Это и
есть гарантия безопасности релиза. Временное staging-дерево удаляется в любом
исходе, в git и в живые клоны артефакты сборки не попадают.

```bash
deploy/local_deploy.sh release                 # origin/dev → staging build → origin/main + рестарт main
deploy/local_deploy.sh release --from <ref>    # выпустить конкретную ветку/коммит вместо dev
deploy/local_deploy.sh release --no-promote    # без пуша: провалидировать и перевыкатить текущий origin/main
...
предком кандидата, команда останавливается ещё до сборки (защита от случайной
перезаписи истории).
