...
## Анонимизированная межпользовательская статистика (FAN-2392)

При установке per-tenant snapshot'а хост сохраняет в security.db
(`global_model_stats`) анонимизированный вклад tenant'а в общий разрез
«Efficiency by models»: только модель, токены по видам, долю story points и
оценочную стоимость — ничего из содержимого задач, проектов, агентов или
аккаунта. Строки ключуются внутренним числовым id tenant'а исключительно для
идемпотентной замены при следующем snapshot'е; API (`/api/global-model-efficiency`)
отдаёт только суммы по моделям без разбивки по пользователям. Удаление
пользователя удаляет и эти строки в той же транзакции `delete_user`.
