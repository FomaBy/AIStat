...
2. взять per-tenant advisory-lock (`data/cli_profiles/conn-<id>.lock`); если он занят
   ...
   же tenant;
3. расшифровать PAT (`get_token`); если его уже нет (revoke между list и get) —
...
7. опубликовать per-tenant snapshot этой БД;
8. `auth logout` + удаление residue профиля;
9. отчитаться о результате (`worker_sync.report_sync`) для кабинета пользователя.
