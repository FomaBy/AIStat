...
const I18N = (() => {
  ...
  const messages = {
    ...
    flowNote: ["Метрики считаются только по наблюдаемым событиям: переходы статусов и снимки флота записываются с момента включения сбора, старая история честно показывается как непокрытая, без приближений. Простой флота — по всему воркспейсу; фильтр проектов на него не влияет.", "Metrics are computed only from observed events: status transitions and fleet snapshots are recorded from the moment collection was enabled; older history is honestly reported as uncovered, with no approximations. Idle fleet is workspace-wide; the project filter does not affect it."],
