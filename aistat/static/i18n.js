...
const I18N = (() => {
  ...
  const messages = {
    ...
    flowMetrics: ["Метрики потока конвейера", "Pipeline flow metrics"],
    flowWindow: ["Окно", "Window"], flowLane: ["Лейн", "Lane"], allLanes: ["Все лейны", "All lanes"],
    flowCycleMedian: ["Цикл: медиана", "Cycle time: median"], flowCycleP90: ["Цикл: p90", "Cycle time: p90"],
    flowRework: ["Доля доработок", "Rework rate"], flowIdle: ["Простой флота", "Idle fleet share"],
    flowReady: ["Готовый фронтир", "Ready frontier"], flowPmP95: ["PM: p95 до готовности", "PM: p95 to ready"],
    flowWaiting: ["Возраст ожидания: медиана", "Waiting age: median"], flowFirstPass: ["QA с первой попытки", "First-pass QA"],
    flowMeasuredCount: ["Измерено задач", "Issues measured"], flowMedian: ["Медиана", "Median"],
    flowMeasuredSub: ["измерено {measured} из {done} завершённых", "measured {measured} of {done} completed"],
    flowP90Sub: ["в работе сейчас: {censored}", "currently in flight: {censored}"],
    flowReworkSub: ["{reworked} из {candidates} кандидатов", "{reworked} of {candidates} candidates"],
    flowIdleSub: ["покрытие {pct}% окна", "{pct}% of the window observed"],
    flowReadySub: ["{ready} карточек · {points} SP", "{ready} cards · {points} SP"],
    flowPmP95Sub: ["измерено {measured} карточек", "measured {measured} cards"],
    flowWaitingSub: ["измерено {waiting} готовых карточек", "measured {waiting} ready cards"],
    flowFirstPassSub: ["{passed} из {denominator} задач", "{passed} of {denominator} issues"],
    flowCoverageEvents: ["переходы статусов наблюдаются с {start}", "status transitions observed since {start}"],
    flowCoverageNoEvents: ["переходы статусов ещё не наблюдались", "status transitions not observed yet"],
    flowCoverageSnapshots: ["снимки флота с {start}", "fleet snapshots since {start}"],
    flowCoverageNoSnapshots: ["снимков флота ещё нет", "no fleet snapshots yet"],
    flowCoverageDetail: ["завершено без наблюдаемого старта: {noStart} · отменено: {cancelled} · вердикты без даты: {unwindowed}", "done without an observed start: {noStart} · cancelled: {cancelled} · verdicts without a date: {unwindowed}"],
    flowAttributionCoverage: ["provenance: неполных {unknown} · legacy unknown {legacy}", "provenance: {unknown} incomplete · {legacy} legacy unknown"],
    sloTitle: ["SLO конвейера и сквозной след", "Pipeline SLOs and end-to-end trace"],
    sloName: ["SLO", "SLO"], sloOwner: ["Владелец", "Owner"],
    sloThreshold: ["Порог", "Threshold"], sloObjective: ["Цель", "Objective"],
    sloActual: ["Факт", "Actual"], sloMeasured: ["Числитель / знаменатель", "Numerator / denominator"],
    sloBudget: ["Бюджет ошибок", "Error budget"], sloAlerts: ["События нарушений", "Breach events"],
    sloUnmeasured: ["не измерено", "not measured"],
    sloNoAlerts: ["нарушений в окне нет", "no breaches in the window"],
    sloAlertLine: ["{severity} · {slo} · владелец {owner} · ключ дедупликации {key}", "{severity} · {slo} · owner {owner} · dedupe key {key}"],
    lineageLookup: ["Сквозной след по correlation ID", "End-to-end trace by correlation id"],
    lineageShow: ["Показать цепочку", "Show chain"],
    lineageStage: ["Этап", "Stage"], lineageStatus: ["Состояние", "State"],
    lineageDetail: ["Ссылки", "Links"],
    lineageNotFound: ["Цепочка по этому correlation ID не наблюдалась", "No chain observed for this correlation id"],
    lineageComplete: ["{id}: цепочка полная", "{id}: chain complete"],
    lineageGaps: ["{id}: пробелы — {gaps}", "{id}: gaps — {gaps}"],
    sloNote: ["Каждый SLO показывает окно, числитель, знаменатель, порог и владельца. Отсутствующая или расходящаяся связь помечается явно (missing / stale) и не заменяется предположением.", "Every SLO states its window, numerator, denominator, threshold and owner. A missing or diverging link is marked explicitly (missing / stale) and never replaced by a guess."],
    flowNote: ["Метрики считаются только по наблюдаемым событиям. До включения provenance старые запуски остаются неизвестными, без догадок. Простой флота — по всему воркспейсу; фильтр проектов на него не влияет.", "Metrics use only observed events. Runs from before provenance collection remain unknown, without guesses. Idle fleet is workspace-wide; the project filter does not affect it."],
    qualityAdjustedCost: ["Качество-скорректированная стоимость", "Quality-adjusted cost"],
    qualityAdjustedSub: ["USD на принятый SP · все попытки включены", "USD per accepted SP · all attempts included"],
    costByLane: ["Стоимость по лейнам", "Cost by lane"],
    acceptedSp: ["Принятые SP", "Accepted SP"], qualityAdjustedHeader: ["Качество-скорр. / SP ≈", "Quality-adj. / SP ≈"],
    costPeriodAllTime: ["период: всё время", "period: all time"],
    costPeriod: ["период: {from} — {to}", "period: {from} — {to}"],
    laneCostNote: ["Разрез по лейну маршрутизации среди задач со story points, загруженной статистикой и запусками. Знаменатель качества — только принятые QA story points; числитель — полная стоимость задачи со всеми попытками и доработками. Лейн с неоценённой моделью помечен * и не показывает частичную сумму как стоимость.", "Breakdown by routing lane across issues with story points, loaded statistics and task runs. The quality denominator is accepted QA story points only; the numerator is the whole issue cost, including every attempt and rework. A lane with an unpriced model is marked * and never reports a partial sum as its cost."],
    rateProvenance: ["Происхождение тарифов", "Rate provenance"],
    rateEffectiveFrom: ["Действует с", "Effective from"], rateInput: ["Ввод / 1M", "Input / 1M"], rateOutput: ["Вывод / 1M", "Output / 1M"], rateSource: ["Источник", "Source"], rateCapturedAt: ["Зафиксировано", "Captured"],
    noPublishedRates: ["Датированные тарифы ещё не опубликованы: стоимость считается по текущему каталогу.", "No dated rates published yet: cost uses the current catalogue."],
    pricingCoverage: ["покрытие: {priced} из {rows} дневных строк оценено, {unpriced} — без тарифа", "coverage: {priced} of {rows} daily rows priced, {unpriced} unpriced"],
    pricingCoverageEmpty: ["покрытие: дневных строк расхода ещё нет", "coverage: no daily usage rows yet"],
    pricingUnavailable: ["Происхождение тарифов недоступно на этом хосте; стоимость по лейнам показана без него.", "Rate provenance is unavailable on this host; cost by lane is shown without it."],
